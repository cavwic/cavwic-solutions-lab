...
export default function SolutionWorkbench({ initialView = "presales" }: Props) {
  ...
  const loadSample = (kind: "ai" | "robot" | "electromechanical") => {
    const sample = createSampleProject(kind);
    setProject(sample);
  ...
  const taskPrompt = useMemo(() => {
    ...
    if (taskKind === "bid") return `使用 $technical-bid-package 处理工作区 ${path} 中项目 ${project.id}。只消费已复核的招标要求和 library 中已核验资料，生成技术方案、响应表、偏离表、部署与验收文件以及 presentation.md。未知、缺证据和商务价格事项必须保留待确认。`;
    return `使用 $solution-workflow 处理工作区 ${path} 中项目 ${project.id}。按售前、招标要求、技术标和中标交底流程检查现状，必要时调用 $tender-requirement-extraction 与 $technical-bid-package。完成后更新 project.json 和 outputs，并给出仍需人工确认的事项。`;
  }, [project.id, project.localPathHint, taskKind, locale]);

  const renderProjectContext = () => <section className="work-section">
    <div className="section-heading"><div><p>PROJECT / BOUNDARY</p><h2>{t.projectContext}</h2></div><span>{project.schemaVersion}</span></div>
  ...
  const renderPresales = () => <>
    ...
    <section className="work-section">
      <div className="section-heading"><div><p>PRE-CALL / CUSTOMER MEETING</p><h2>{t.presalesPack}</h2></div><button className="icon-command" type="button" onClick={() => addDeliverable("presales")} title={t.add}><Plus size={18}/></button></div>
      <div className="deliverable-list">{project.deliverables.filter((item) => item.stage === "presales").map((item) => <div className="deliverable-row" key={item.id}>
        <select aria-label="Status" value={item.status} onChange={(event) => updateProject("deliverables", project.deliverables.map((candidate) => candidate.id === item.id ? { ...candidate, status: event.target.value as Deliverable["status"] } : candidate))}><option value="not-started">{locale === "zh" ? "未开始" : "Not started"}</option><option value="draft">{locale === "zh" ? "草稿" : "Draft"}</option><option value="review">{locale === "zh" ? "审阅" : "Review"}</option><option value="approved">{locale === "zh" ? "已批准" : "Approved"}</option></select>
        ...
        <button type="button" className="row-delete" title="Remove" onClick={() => updateProject("deliverables", project.deliverables.filter((candidate) => candidate.id !== item.id))}><Trash2 size={16}/></button>
      </div>)}</div>
    </section>
    <section className="work-section two-column-section">
      <div><div className="section-heading"><div><p>POC / DEMO</p><h2>{t.poc}</h2></div></div><div className="stack-fields">
  ...
  const renderRequirements = () => <>
    <section className="work-section">
      <div className="section-heading"><div><p>SOURCE / TRACEABILITY</p><h2>{t.sourceLibrary}</h2></div><button className="command-button" type="button" disabled={busy} onClick={() => sourceInput.current?.click()}><Upload size={17}/>{t.importSources}</button></div>
    ...
    <section className="work-section source-review-grid">
      <div className="source-pane"><div className="pane-title"><div><p>SOURCE</p><h2>{t.sourceSegments}</h2></div>{selectedSource && <span>{selectedSource.segments.length}</span>}</div>
      ...
      <div className="requirements-pane"><div className="pane-title"><div><p>REVIEW QUEUE</p><h2>{t.requirementReview}</h2></div><button className="icon-command" type="button" title={t.add} onClick={() => { const item = createRequirement("tender"); updateProject("requirements", [...project.requirements, item]); setSelectedRequirementId(item.id); }}><Plus size={18}/></button></div>
        ...
        {selectedRequirement ? <div className="requirement-editor">
          <div className="editor-toolbar"><span>{selectedRequirement.id}</span><button type="button" title="Remove" onClick={() => updateProject("requirements", project.requirements.filter((item) => item.id !== selectedRequirement.id))}><Trash2 size={16}/></button></div>
          ...
          <div className="field-grid compact"><Field label={locale === "zh" ? "基线" : "Baseline"}><select value={selectedRequirement.baseline} onChange={(event) => updateRequirement(selectedRequirement.id, { baseline: event.target.value as Requirement["baseline"] })}><option value="discovery">Discovery</option><option value="tender">Tender</option></select></Field><Field label={locale === "zh" ? "分类" : "Category"}><select value={selectedRequirement.category} onChange={(event) => updateRequirement(selectedRequirement.id, { category: event.target.value as Requirement["category"] })}>{Object.entries(categoryLabels[locale]).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></Field></div>
  ...
  return <div className="solution-app" data-ready={ready ? "true" : "false"}>
    <header className="project-header">
      <div><p>SOLUTION WORKSPACE / LOCAL FIRST</p><h1>{t.project}</h1><span>{project.name}</span></div>
    ...
    <nav className="workspace-toolbar" aria-label="Workspace actions"><button type="button" aria-label={`${t.sample} / AI`} onClick={() => loadSample("ai")} title={`${t.sample} / AI`}><Database size={17}/><span>{t.sample} / AI</span></button><button type="button" aria-label="Robot sample" onClick={() => loadSample("robot")} title="Robot sample"><Database size={17}/><span>Robot</span></button><button type="button" aria-label="Electromechanical sample" onClick={() => loadSample("electromechanical")} title="Electromechanical sample"><Database size={17}/><span>Electromechanical</span></button><button type="button" aria-label={t.reset} onClick={() => setProject(createEmptyProject(locale))} title={t.reset}><RotateCcw size={17}/><span>{t.reset}</span></button><button className="toolbar-spacer" type="button" aria-label={theme === "light" ? "Dark mode" : "Light mode"} onClick={switchTheme} title={theme === "light" ? "Dark mode" : "Light mode"}>{theme === "light" ? <Moon size={17}/> : <Sun size={17}/>}</button><button type="button" aria-label={locale === "zh" ? "EN" : "中"} onClick={switchLocale} title={locale === "zh" ? "English" : "中文"}><Languages size={17}/><span>{locale === "zh" ? "EN" : "中"}</span></button></nav>
    <div className="workspace-shell">
      <aside className="stage-rail" aria-label="Solution lifecycle">{viewMeta.map((item) => { const Icon = item.icon; return <button type="button" aria-label={t[item.id]} title={t[item.id]} className={view === item.id ? "active" : ""} key={item.id} onClick={() => setView(item.id)}><span>{item.code}</span><Icon size={19}/><strong>{t[item.id]}</strong><ChevronRight size={16}/></button>; })}<div className="rail-status"><p>{locale === "zh" ? "当前阶段" : "Current stage"}</p><strong>{project.stage.toUpperCase()}</strong><span>{issues.filter((item) => item.severity === "error").length} {locale === "zh" ? "个阻断项" : "blocking issues"}</span></div></aside>
