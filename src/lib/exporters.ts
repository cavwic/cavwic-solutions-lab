  ...
  const document = new Document({
    ...
    sections: [{ children: [
      ...
      ...project.presalesRounds.flatMap((round) => [
        ...
        new Paragraph({ text: `参会人员：${round.participants.map((participant) => `${participant.name}（${participantCategoryZh[participant.category]}）`).join("、") || "待确认"}` }),
